...
void Clock2_1_Start(void) 
{
    ...
    Clock2_1_CLKEN |= Clock2_1_CLKEN_MASK;
	Clock2_1_CLKSTBY |= Clock2_1_CLKSTBY_MASK;
}
...
void Clock2_1_Stop(void) 
{
    ...
    Clock2_1_CLKEN &= (uint8)(~Clock2_1_CLKEN_MASK);
	Clock2_1_CLKSTBY &= (uint8)(~Clock2_1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock2_1_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock2_1_StopBlock(void) 
{
    if ((Clock2_1_CLKEN & Clock2_1_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock2_1__CFG3)
        CLK_DIST_AMASK = Clock2_1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock2_1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock2_1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock2_1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock2_1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock2_1_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock2_1_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock2_1_CLKSTBY &= (uint8)(~Clock2_1_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock2_1_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock2_1_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock2_1_GetSourceRegister();
    uint16 oldDivider = Clock2_1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock2_1_CLKEN & Clock2_1_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock2_1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Clock2_1_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock2_1_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock2_1_SetModeRegister(uint8 modeBitMask) 
{
    Clock2_1_MOD_SRC |= modeBitMask & (uint8)Clock2_1_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock2_1_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock2_1_ClearModeRegister(uint8 modeBitMask) 
{
    Clock2_1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock2_1_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock2_1_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock2_1_GetModeRegister(void) 
{
    return Clock2_1_MOD_SRC & (uint8)(Clock2_1_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock2_1_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock2_1_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock2_1_GetDividerRegister();
    uint8 oldSrc = Clock2_1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock2_1_MOD_SRC |= CYCLK_SSS;
        Clock2_1_MOD_SRC =
            (Clock2_1_MOD_SRC & (uint8)(~Clock2_1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock2_1_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock2_1_GetSourceRegister(void) 
{
    return Clock2_1_MOD_SRC & Clock2_1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock2_1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock2_1_SetPhaseRegister(uint8 clkPhase) 
{
    Clock2_1_PHASE = clkPhase & Clock2_1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock2_1_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock2_1_GetPhaseRegister(void) 
{
    return Clock2_1_PHASE & Clock2_1_PHASE_MASK;
}
